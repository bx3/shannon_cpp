...
def analyzer_blat_noExp(Repeats_File,Dest_File,exp_file,N):
    ...
    for (i,line) in enumerate(lines):
        ...
        z = best_rec_dict.get(org,[None,0]);                
        if rec_len>z[1]:
            best_rec_dict[org] = [rec,rec_len,tr_len,rec_tr_len, num_block, blockSizes, qStarts]
    perf = 0
    sum_ret = 0
    no_tr_rec = 0
    total_rec_num_base = 0
    total_tr_num_base = 0 
    total_match_num_base = 0
    align_file = open(Dest_File + '_align', 'w')
    with open(Dest_File,'w') as write_file:
        for (org,z) in best_rec_dict.iteritems():
            write_file.write(org+'\t'+str(z[0])+'\t'+str(z[1])+'\t'+str(z[2]) + '\t' + str(z[3])+ '\t'+ z[4] + '\t'+ z[5] + '\t'+ z[6] + '\n')
            if z[1] >= 0.9*z[2]:
		total_match_num_base += int(z[1])
		total_tr_num_base += int(z[2])
		total_rec_num_base += int(z[3])		
                no_tr_rec +=1
		align_file.write(org+'\t'+str(z[0])+'\t'+str(z[1])+'\t'+str(z[2]) + '\t' + str(z[3]) + '\t'+ z[4] + '\t'+ z[5] + '\t'+ z[6]+'\n')
        ...
        #else:
        #    write_file.write("#Average fractional contig of transcripts retrived:" + '\t' + str(perf) + '  best_rec=0' + '\n')
	total_match_num_base = round(total_match_num_base/1024.0*10)/10.0
	total_rec_num_base = round(total_rec_num_base/1024.0*10)/10.0
	total_tr_num_base = round(total_tr_num_base/1024.0*10)/10.0

	write_file.write("# total size match    " + '\t' + str(total_match_num_base)+'KiB\n')
	write_file.write("# total size recover  " + '\t' + str(total_rec_num_base)+'KiB\n')
	write_file.write("# total size true ref " + '\t' + str(total_tr_num_base)+'KiB\n')
