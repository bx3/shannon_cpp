...
map<std::string,std::string> Fasta_reader::retrieve_all_seqs_hash() {
    
    std::map<std::string,std::string> all_seqs_hash;
    
    while (this->hasNext()) {
        Fasta_entry f = this->getNext();
        std::string acc = f.get_accession();
        std::string seq = f.get_sequence();
